...
export default function NewArrival() {
  ...
  return (
    <section className=" relative mx-auto max-w-7xl px-4 sm:static sm:px-6 lg:px-0 pb-16 flex flex-col">
      <div className="container mx-auto mb-4">
        <div className="flex mx-auto max-w-7xl px-4 sm:static sm:px-6 lg:px-8 mb-4 pt-16">
          <h3 className="text-3xl capitalize font-bold">
            New Arrival <br className="" />this summer for you
          </h3>
        </div>
      </div>
      <div className="overflow-x-hidden px-4" id="carousel">
        ...
        <div className="flex -mx-4 flex-row relative">
          {products.map((products)=>(
            <div className="px-4 relative card group" key={products.id}>
            <div
              className="rounded-xl overflow-hidden card-shadow relative"
              style={{width: 287, height: 386}}
            >
              <div
                className="absolute opacity-0 group-hover:opacity-100 transition duration-200 flex items-center justify-center w-full h-full bg-black bg-opacity-20"
              >
                <div
            ...
            <h5 className="text-lg font-semibold mt-4">{products.name}</h5>
            <span className="">{products.price}</span>
            <Link to={`/categories/${products.id}/products/${products.id}`} className="stretched-link">
            </Link>
          </div>
          ))}
        </div>
